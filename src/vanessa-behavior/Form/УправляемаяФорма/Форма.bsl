...
// Функция для поиска реквизита в конкретной тестируемой форме.
//
// Параметры:
//  ТекущаяФорма		 - ТестируемаяФорма	 - форма, в которой требуется найти реквизит.
//  ИмяРеквизита		 - Строка			 - имя или заголовок реквизита.
//  ИскатьПоИмени		 - Булево			 - если Истина, то поиска будет производиться по имени реквизита,
//                                               если Ложь, то по заголовку.
//  ВызыватьИсключение	 - Булево			 - в случае, когда реквизит не найден, при Истина вызывается исключение,
//                                               при Ложь возвращается Неопределено.
//  ТипПоля				 - Тип				 - тип объекта реквизита в тестируемой форме для фильтрации поиска.
//  ДопПараметры		 - Структура		 - структура для дополнительных условий поиска со свойствами:
//    * ИскатьТолькоПоляВКоторыеВозможенВвод - Булево - из найденных элементов будут исключены поля надписи, картинки, флажка и переключателя.
//    * ИскатьТолькоПоляНадписей			 - Булево - в найденных элементах будет оставлены только надписи.
// Возвращаемое значение:
//  ТестируемаяФорма, ТестируемаяГруппаФормы,
//  ТестируемаяКнопкаФормы, ТестируемаяТаблицаФормы,
//  ТестируемоеПолеФормы, ТестируемаяДекорацияФормы,
//  Неопределено							- найденный реквизит формы или Неопределено.
//
&НаКлиенте
Функция НайтиРеквизитФормыПоЗаголовку(ТекущаяФорма, ИмяРеквизита, ИскатьПоИмени = Ложь,
	...
	Если МассивЭлементов = Неопределено Тогда
		НашлиЭлемент = Ложь;
	ИначеЕсли МассивЭлементов.Количество() = 0 Тогда
		НашлиЭлемент = Ложь;
	Иначе
	...
	Если НашлиЭлемент Тогда
		ПолеРеквизит = МассивЭлементов[0];
		Возврат ПолеРеквизит;
	Иначе
		Если ВызыватьИсключение Тогда
			Если ИскатьПоИмени Тогда
				ТекстСообщения = "Элемента формы <%1> с именем <%2> не найдено.";
			Иначе	
				ТекстСообщения = "Элемента формы <%1> с заголовком <%2> не найдено.";
			КонецЕсли;
...
Процедура УдалитьИзМассиваПоляТаблицTestClient(ТекущаяФорма, МассивЭлементов)
	...
	Для каждого ТаблицаФормы Из ТаблицыФормы Цикл
		...
		ОчиститьМассивИД(МассивИндексов, МассивЭлементов);
		МассивИндексов = Новый Массив;
	КонецЦикла;
...
Процедура ОставитьВМассивеТолькоПоляНадписей(МассивЭлементов)
	...
	Если МассивИндексов.Количество() > 0 Тогда
		Для Ккк = 0 По МассивИндексов.Количество() - 1 Цикл
			МассивЭлементов.Удалить(МассивИндексов[МассивИндексов.Количество() - Ккк - 1]);
		КонецЦикла;
	КонецЕсли;
...
Процедура НайтиРеквизитВМассивеФорм(МассивФорм, ПолеРеквизит, ИмяРеквизита, ИскатьПоИмени, ТипПоля, ДопПараметры)
	...
	Для каждого ТекущаяФорма Из МассивФорм Цикл
		...
		// ВызыватьИсключение = Ложь, так как обходим массив.
		ПолеРеквизит = НайтиРеквизитФормыПоЗаголовку(ТекущаяФорма, ИмяРеквизита, ИскатьПоИмени, Ложь, ТипПоля, ДопПараметры);
		Если ПолеРеквизит = Неопределено Тогда
			Продолжить;
		Иначе
			Возврат;
